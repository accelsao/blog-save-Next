...
## Computing the Analytic Gradient with Backpropagation with Softmax Linear Classifier


$$p_k = \frac{e^{f_k}}{ \sum_j e^{f_j} } \hspace{1in} L_i =-\log\left(p_{y_i}\right)$$


$$L_i =-\log(\frac{e^{f_k}}{ \sum_j e^{f_j} })$$

$$L_i =-\log({e^{f_k}})+\log({ \sum_j e^{f_j} })$$

$$\\\frac{\partial L_i }{ \partial f_k }=-\frac{e^{f_k}}{e^{f_k}}+\frac{e^{f_k}}{\sum_j e^{f_j}}$$
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        softmax_scores = np.exp(shift_scores) / np.sum(np.exp(shift_scores), axis=1,keepdims=1)

        # Calculate our cross entropy Loss.
        # shift_scores.shape = (N,C)
        # y shape(N,)
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            random_batch=np.random.permutation(num_train)[:batch_size]
